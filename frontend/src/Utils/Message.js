export const fetchUsers = async (search = "") => {
  try {
    const res = await fetch(
      `${import.meta.env.VITE_BACKEND_URL}/auth/users?search=${search}`,
      {
        credentials: "include",
      }
    );
...
export const deleteMessage = async (messageId, receiverId) => {
  try {
    const res = await fetch(
      `${import.meta.env.VITE_BACKEND_URL}/message/delete/${messageId}`,
      {
        method: "DELETE",
...
export const updateMessage = async (messageId, newContent, receiverId) => {
  try {
    const res = await fetch(
      `${import.meta.env.VITE_BACKEND_URL}/message/update/${messageId}`,
      {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
